...
#'
#' @export
C3BarChart <- function(message,title, xLab, yLab,xLabelPos='outer-center', yLabelPos='outer-middle',rotate=0, width = NULL, height = NULL, elementId = NULL) {

  tab <- as.data.frame(message)    

  # forward options using x
  x = list(
          list(data = tab),
          list(title=title,ylab=yLab, xlab=xLab, xlabPos = xLabelPos, ylabPos=yLabelPos, rotate=rotate)
          )
